    ...
    <a-layout-content class="content">
      <a-col :span="11" class="content-left">
        ...
        <a-col :span="24" class="filter-buttons">
          <a-button-group>
            <a-button type="default" icon="upload" @click="upload"
              >上传</a-button
            >
            <a-button type="primary" icon="download" @click="download"
              >下载</a-button
            >
...
export default {
  ...
  computed: {
    filterStr() {
      return `.filter-image { 
        filter: blur(${this.filter.blur}px) brightness(${this.filter.brightness}) contrast(${this.filter.contrast}%) grayscale(${this.filter.grayscale}%) hue-rotate(${this.filter.hueRotate}deg) 
      invert(${this.filter.invert}%) opacity(${this.filter.opacity}%) saturate(${this.filter.saturate}%) sepia(${this.filter.sepia}%) drop-shadow(${this.filter.offsetX}px ${this.filter.offsetY}px ${this.filter.blurRadius}px ${this.filter.color}); 
    }`;
    },
  },
  methods: {
    ...
    download() {
      ...
      const filterStr = `blur(${this.filter.blur}px) brightness(${this.filter.brightness}) contrast(${this.filter.contrast}%) grayscale(${this.filter.grayscale}%) hue-rotate(${this.filter.hueRotate}deg) 
      invert(${this.filter.invert}%) opacity(${this.filter.opacity}%) saturate(${this.filter.saturate}%) sepia(${this.filter.sepia}%) drop-shadow(${this.filter.offsetX}px ${this.filter.offsetY}px ${this.filter.blurRadius}px ${this.filter.color})
    `;
